...
### Report-template fidelity across agents

A skill step that reports structured data (scores, citations, rationale) must not require the model to recall that data more than about one step after it was produced. Claude Code's long-context fidelity can paper over a long gap between computing something and reporting it; other hosted agents (observed directly with `tailor-resume`'s job-match score under Codex CLI and Gemini CLI) degrade — dropping sub-scores/rationale entirely, or replacing specific evidence with vague generic text — the further that data has to travel through unrelated intervening steps. The fix isn't "use a smarter model," it's structural: either format-and-report the data immediately after computing it, or persist it to a file at computation time and have the report step re-read that file mechanically rather than recall it. `setup-opencode.md` already documents the same effect for local Ollama models under long, dense instruction sequences ("a real limitation of the model, not a bug in this repo's skills") — this is the same failure shape on hosted models, just triggered by distance-to-report rather than raw model size, and it was previously undiagnosed and unconnected to that framing. `setup-openai-codex.md`, `setup-gh-copilot-cli.md`, and `setup-gemini-cli.md` had already independently logged symptoms of it (dropped sub-scores, missing salary range, unexplained excluded experience) before the cause was identified.

Audit of every skill with a report-template step, and its distance from where the reported data was computed:
- `tailor-resume` — **was the worst case, fixed.** Its job-match score (formerly Step 2b) and salary analysis (formerly Step 2c) are now each their own skill (`score-job-match`, `analyze-salary`, see "Invoking one skill from another" below) that persists its full result to a fixed scratch file the moment it's computed; `tailor-resume`'s Step 9 (manifest write) and Step 11 (report) both re-read those files mechanically rather than recalling values from earlier in the run.
- `ats-validate`, `update-status` — compliant: computation and reporting happen in adjacent steps.
- `prep-interview`, `learn-preferences` — low risk: the report step summarizes a file the immediately-preceding step just wrote, not something computed further back.
- `applied` — low risk: `match_score` is copied from a manifest re-read in Step 2, not fresh judgment produced in that run.
- `import-applications`, `match-resume-style` — low risk: each has an explicit "show and confirm" step that re-surfaces the data shortly before it's written/reported.
- `find-job-descriptions` — **fixed.** Its per-candidate score comes from `score-job-match`; Step 6 now copies `formatted_report` (plus all four sub-scores and `interpretation`, not just the bare `total`) onto the candidate object immediately after each invocation, Step 8 persists all of it into `job-search-seen.json`'s ledger (previously only `score`/`saved`/`full_text_fetched` survived), and Step 9 is instructed to draw its per-candidate rationale from that persisted `formatted_report` rather than recalling or inventing one. This also closed a second, related gap: a *cached* candidate (scored in a prior run) previously had no rationale data at all to report from, even in Claude Code — Step 6 now force-rescores a cached candidate that predates this schema so its `formatted_report` gets backfilled.

### Invoking one skill from another

Several skills call another skill as a subroutine mid-procedure — "Invoke the `load-career-profile` skill in `full` mode," "Invoke the `score-job-match` skill," and similarly in `career-coach`, `find-job-descriptions`, `prep-interview`, `match-resume-style`, and `analyze-salary`. Claude Code has a native tool for this that loads the target skill's instructions into the current turn. Agents without an equivalent mechanism should simulate it manually: open and read the target skill's `SKILL.md` file, find the matching `## Mode: <name>` section if the invocation names one (mode names are consistently `## Mode: <name>` headers, e.g. in `load-career-profile/SKILL.md`), execute those instructions inline as part of the current procedure, then resume the caller's next step. This applies uniformly to every "invoke skill X" reference in this repo — one explanation covers all of them.
...
- **`.claude/skills/*/SKILL.md`** — The skill definitions themselves (Agent Skills format — see "Supported AI Coding Agents" above). `.claude/skills/load-career-profile/SKILL.md` (loads `template/` data), `.claude/skills/score-job-match/SKILL.md` (the job-match rubric, see "Report-template fidelity across agents" above), and `.claude/skills/analyze-salary/SKILL.md` (the asking-salary recommendation) are internal helper skills invoked by several of the others; none are meant for direct end-user invocation.

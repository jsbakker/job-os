...
- `find-job-descriptions` — **flagged, not yet fixed.** Its per-candidate score now comes from `score-job-match` (so the arithmetic/formatting fix applies), but the per-candidate rationale still isn't persisted anywhere before its own multi-step-later report (Step 9) — `job-search-seen.json`'s ledger keeps only the numeric `score`. Same failure shape as `tailor-resume` had; a fast-follow using the identical persist-and-reread pattern (add a `rationale`/`formatted_report`-equivalent field to the ledger) is the natural next fix, not done here.

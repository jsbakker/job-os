...
Also computes two salary-adjacent things for contract/hourly job postings: `contract-rate` derives a billing-rate anchor from the applicant's FTE salary using the standard 1.5x-2x contractor markup (self-employment tax, self-funded benefits, no PTO, income gaps) rather than trusting salary-aggregator sites, which report realized contractor income, not billing rates. `net-pay`/`net-pay-compare` estimate after-tax take-home using real marginal tax brackets, so a nominal raise that's mostly eaten by a higher bracket (or lost benefits when moving to contract status) doesn't look better than it is. Tax-bracket data is a small, explicitly-dated set of jurisdictions (see `JURISDICTIONS` in the script; currently two Canadian provinces, tax year 2026) that any adopter can extend for their own province/state/country — an unsupported jurisdiction is skipped rather than guessed at.
